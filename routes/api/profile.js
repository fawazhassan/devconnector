...
// @desc    View specific user profile using handle to identify unique user
// @access  Public
...
// @desc    View specific user profile using user_id
// @access  Public

router.get("/user/:user_id", (req, res) => {
  const errors = {};
  Profile.findOne({ user: req.params.user_id }, err => {
    if (err) {
...
router.post(
  ...
  passport.authenticate("jwt", { session: false }),
  (req, res) => {
    //Deconstruct errors and isValid Boolean
    const { errors, isValid } = validateExperienceInput(req.body);

    if (!isValid) {
      //return errors and set 400 status
      return res.status(400).json(errors);
    }
